...
function sfsi_plus_options_updater2() {
	...
	$sfsi_plus_threads_page    = isset( $_POST["sfsi_plus_threads_page"] ) ? sanitize_text_field( $_POST["sfsi_plus_threads_page"] ) : 'no';
	$sfsi_plus_threads_pageUrl = isset( $_POST["sfsi_plus_threads_pageUrl"] ) ? esc_url( trim( $_POST["sfsi_plus_threads_pageUrl"] ) ) : '';
	$sfsi_plus_threads_share   = isset( $_POST["sfsi_plus_threads_share"] ) ? sanitize_text_field( $_POST["sfsi_plus_threads_share"] ) : 'no';

	$sfsi_plus_bluesky_page    = isset( $_POST["sfsi_plus_bluesky_page"] ) ? sanitize_text_field( $_POST["sfsi_plus_bluesky_page"] ) : 'no';
	$sfsi_plus_bluesky_pageUrl = isset( $_POST["sfsi_plus_bluesky_pageUrl"] ) ? esc_url( trim( $_POST["sfsi_plus_bluesky_pageUrl"] ) ) : '';
	$sfsi_plus_bluesky_share   = isset( $_POST["sfsi_plus_bluesky_share"] ) ? sanitize_text_field( $_POST["sfsi_plus_bluesky_share"] ) : 'no';
	...
	$up_option2 = array(
		...
		/* threads buttons options */
		'sfsi_plus_threads_page'                => sanitize_text_field( $sfsi_plus_threads_page ),
		'sfsi_plus_threads_pageUrl'             => esc_url( $sfsi_plus_threads_pageUrl ),
		'sfsi_plus_threads_share'               => sanitize_text_field( $sfsi_plus_threads_share ),

		/* bluesky buttons options */
		'sfsi_plus_bluesky_page'                => sanitize_text_field( $sfsi_plus_bluesky_page ),
		'sfsi_plus_bluesky_pageUrl'             => esc_url( $sfsi_plus_bluesky_pageUrl ),
		'sfsi_plus_bluesky_share'               => sanitize_text_field( $sfsi_plus_bluesky_share ),
...
function sfsi_plus_options_updater3() {
	...
	/* Flat color settings */
	$prefixes = ['rss', 'email', 'facebook', 'twitter', 'youtube', 'linkedin', 'pinterest', 'instagram', 'bluesky', 'threads', 'houzz', 'ok', 'telegram', 'vk', 'wechat', 'whatsapp', 'weibo', 'copylink', 'mastodon', 'ria', 'inha'];

	foreach ($prefixes as $prefix) {
		$variableName = "sfsi_plus_{$prefix}_bgColor";
		$$variableName = isset($_POST[$variableName]) ? sanitize_text_field($_POST[$variableName]) : '';
	}

	$customColorsData = [];
	foreach ($prefixes as $prefix) {
		$variableName = "sfsi_plus_{$prefix}_bgColor";
		$customColorsData[$variableName] = sanitize_text_field($$variableName);
	}
...
function sfsi_plus_options_updater5() {
	...
	$sfsi_plus_threadsIcon_order = isset( $_POST["sfsi_plus_threadsIcon_order"] ) ? intval( $_POST["sfsi_plus_threadsIcon_order"] ) : '35';
	$sfsi_plus_blueskyIcon_order = isset( $_POST["sfsi_plus_blueskyIcon_order"] ) ? intval( $_POST["sfsi_plus_blueskyIcon_order"] ) : '36';
	...
	$up_option5 = array(
		...
		'sfsi_plus_threadsIcon_order'     => intval( $sfsi_plus_threadsIcon_order ),
		'sfsi_plus_blueskyIcon_order'     => intval( $sfsi_plus_blueskyIcon_order ),
...
function sfsi_plus_getCounts( $for_resposive = false ) {
	...
	if ( $for_resposive == false ) {
		...
		/* get threads counts */
		if ( isset( $option4['sfsi_plus_threads_countsFrom'] ) && ! empty( $option4['sfsi_plus_threads_countsFrom'] ) && $option4['sfsi_plus_threads_countsFrom'] == "manual" ) {
			$scounts['threads_count'] = $option4['sfsi_plus_threads_manualCounts'];
		}

		/* get bluesky counts */
		if ( isset( $option4['sfsi_plus_bluesky_countsFrom'] ) && ! empty( $option4['sfsi_plus_bluesky_countsFrom'] ) && $option4['sfsi_plus_bluesky_countsFrom'] == "manual" ) {
...
function sfsi_plus_save_export() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_save_export' ) ) {
...
function sfsi_plus_install_newsletter() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_install_newsletter' ) ) {
		return wp_send_json_error();
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		return wp_send_json_error();
...
function sfsi_plus_installDate() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_installDate' ) ) {
		return wp_send_json_error();
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		return wp_send_json_error();
...
function sfsi_plus_currentDate() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_currentDate' ) ) {
		return wp_send_json_error();
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		return wp_send_json_error();
...
function sfsi_plus_showNextBannerDate() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_showNextBannerDate' ) ) {
		return wp_send_json_error();
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		return wp_send_json_error();
...
function sfsi_plus_cycleDate() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_cycleDate' ) ) {
		return wp_send_json_error();
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		return wp_send_json_error();
...
function sfsi_plus_loyaltyDate() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_loyaltyDate' ) ) {
		return wp_send_json_error();
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		return wp_send_json_error();
...
function sfsi_plus_banner_global_pinterest() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_banner_global_pinterest' ) ) {
		return wp_send_json_error();
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		return wp_send_json_error();
...
function sfsi_plus_banner_global_firsttime_offer() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_banner_global_firsttime_offer' ) ) {
		return wp_send_json_error();
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		return wp_send_json_error();
...
function sfsi_plus_banner_global_social() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_banner_global_social' ) ) {
		return wp_send_json_error();
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		return wp_send_json_error();
...
function sfsi_plus_banner_global_load_faster() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_banner_global_load_faster' ) ) {
		return wp_send_json_error();
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		return wp_send_json_error();
...
function sfsi_plus_banner_global_shares() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_banner_global_shares' ) ) {
		return wp_send_json_error();
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		return wp_send_json_error();
...
function sfsi_plus_banner_global_gdpr() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_banner_global_gdpr' ) ) {
		return wp_send_json_error();
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		return wp_send_json_error();
...
function sfsi_plus_banner_global_http() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_banner_global_http' ) ) {
		return wp_send_json_error();
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		return wp_send_json_error();
...
function sfsi_plus_banner_global_upgrade() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'sfsi_plus_banner_global_upgrade' ) ) {
		return wp_send_json_error();
	}
	if ( ! current_user_can( 'manage_options' ) ) {
		return wp_send_json_error();
